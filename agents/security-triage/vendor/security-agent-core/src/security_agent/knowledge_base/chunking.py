...
# Upstream: knowledge_base/chunking.py @ 380554275fdfcf177737253132bd094178314078
# See the repo-root NOTICE and knowledge_base/curation/LICENSE-redamon.
# Local changes: _load_chunking_defaults() reads knowledge_base.settings instead of kb_config.
...
def _load_chunking_defaults() -> tuple[int, int]:
    """
    Load (max_tokens, preferred_tokens) from the knowledge_base config with a safe fallback.
    ...
    Reading from config.yaml means swapping to a longer-context embedder is a
    one-line YAML edit instead of a code change.

    Falls back to (480, 256) — sized for a 512-token embedder — if config loading
    fails for any reason (missing yaml, import error during early bootstrap,
    corrupted config).

    This runs at class-definition time (see ChunkStrategy below), so every import of every
    curation client depends on it. It must never raise — hence the bare except and the
    exception-safe `load_kb_settings`.
    """
    try:
        from security_agent.knowledge_base.settings import load_kb_settings
        cfg = load_kb_settings()
        return cfg.max_chunk_tokens, cfg.preferred_chunk_tokens
    except Exception:
        return 480, 256
...
class ChunkStrategy:
    ...
    def generate_chunk_id(source: str, unique_key: str) -> str:
        ...
        Args:
            source: Source identifier (e.g., 'nvd', 'cwe').
    ...
    def chunk_structured(self, entries: list[dict]) -> list[dict]:
        ...
        Used for: CVEs, CWE entries, ExploitDB descriptions,
        tool flag groups.
    ...
    def chunk_markdown(self, text: str, split_on: str = "##") -> list[dict]:
        ...
        Used for: long-form reference docs and exploit writeups.
