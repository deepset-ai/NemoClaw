...
# Upstream: knowledge_base/curation/nvd_client.py @ 380554275fdfcf177737253132bd094178314078
# See the repo-root NOTICE and knowledge_base/curation/LICENSE-redamon.
# Local changes: imports rewired to security_agent.knowledge_base.*; the default
# cache_dir and the source_path literals now resolve under <project>/data/kb_cache/.
...
from security_agent.knowledge_base.atomic_io import atomic_write_json
...
from security_agent.knowledge_base.curation.safe_http import MAX_NVD_PAGE_BYTES, safe_get
...
NVD_SOURCE_PATH = f"data/kb_cache/nvd/{UNIFIED_CACHE_FILENAME}"
...
class NVDClient(BaseClient):
    ...
    def fetch(
        ...
        **kwargs,
        ...
        # Resolve lookback window: kwarg → env var → config.yaml → default
        if nvd_days is None:
            ...
            if env_val is not None:
                ...
                try:
                    nvd_days = load_kb_settings().nvd_lookback_days
                except Exception:
        ...
        if nvd_min_cvss is None:
            ...
            if env_val is not None:
                ...
                try:
                    nvd_min_cvss = load_kb_settings().nvd_min_cvss
                except Exception:
                    nvd_min_cvss = 7.0
    ...
    def _load_unified_cache(self) -> dict[str, dict]:
        ...
        Returns an empty dict if nothing is on disk. (Upstream also merged
        pre-unification per-profile cache files here; this project starts clean, so that
        migration path was dropped.)
        """
